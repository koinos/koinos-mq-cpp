  ...
  try
  {
    ...
    else
    {
      for( const auto& h_pair: routing_itr->second )
      {
        ...
        std::visit( util::overloaded{ [ & ]( const msg_handler_string_func& f )
                                      {
                                        ...
                                        if( msg->reply_to.has_value() && msg->correlation_id.has_value() )
                                        {
                                          ...
                                          boost::asio::dispatch( _ioc,
                                                                 std::bind( &request_handler::publish, this, reply ) );
                                        }
                                      },
...
request_handler::request_handler( boost::asio::io_context& io_context ):
    ...
    _ioc( io_context ),
    _signals( io_context ),
    _retryer( io_context, std::chrono::milliseconds( 30'000 ) )
{
  _signals.add( SIGINT );
...
void request_handler::connect( const std::string& amqp_url, retry_policy policy )
{
  ...
  error_code code = _retryer.with_policy(
    ...
    [ & ]() -> error_code
    {
      ...
      e = _consumer_broker->connect( _amqp_url,
...
error_code request_handler::on_connect( message_broker& m )
{
  ...
  for( const auto& msg_handler: _message_handlers )
  {
    ...
    {
      ec = _consumer_broker->declare_exchange( msg_handler.exchange,
                                               msg_handler.competing_consumer ? exchange_type::direct
                                                                              : exchange_type::topic,
                                               false, // Passive
                                               true,  // Durable
                                               false, // Auto Delete
      ...
      auto queue_res = _consumer_broker->declare_queue( msg_handler.competing_consumer ? msg_handler.routing_key : "",
                                                        false,                           // Passive
    ...
    {
      _handler_map.emplace( binding,
                            std::vector< handler_pair >{ std::make_pair( msg_handler.verify, msg_handler.handler ) } );
      _handler_map.emplace( default_binding,
                            std::vector< handler_pair >{ std::make_pair( msg_handler.verify, msg_handler.handler ) } );
    }
    else
    {
...
void request_handler::add_broadcast_handler( const std::string& routing_key,
...
void request_handler::add_rpc_handler( const std::string& service,
...
void request_handler::add_msg_handler( const std::string& exchange,
                                       const std::string& routing_key,
                                       bool competing_consumer,
                                       handler_verify_func verify,
                                       msg_handler_func handler )
{
  KOINOS_ASSERT( !connected(),
                 request_handler_already_connected,
                 "message handlers should be added to the request handler prior to amqp connection" );

  _message_handlers.push_back( { exchange, routing_key, competing_consumer, verify, handler } );
}
